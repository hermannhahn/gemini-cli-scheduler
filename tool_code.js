...
const PERSISTENCE_PATH = process.env.SCHEDULER_PATH || path.join(EXTENSION_DIR, "tasks.json");
...
function detectEnabledExtensions() {
  try {
    ...
    for (const section of sections) {
      ...
      const isEnabled = section.includes("Enabled (User): true") || section.includes("Enabled (Workspace): true");
...
function scheduleTask(task) {
  ...
  if (executeAt < new Date()) {
    logToFile(`SYSTEM: Skipping task "${task.name}" because it's in the past.`);
    task.status = 'missed';
    saveTasks();
    return;
  }
...
function executeHeadless(task) {
  ...
  if (finalExtensions.length > 0) {
    logStream.write(`Enabled Extensions: ${finalExtensions.join(", ")}\n\n`);
  } else {
...
function cancelTask(idOrName) {
  const taskIndex = tasks.findIndex(t => t.id === idOrName || t.name === idOrName);
  if (taskIndex !== -1) {
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      {
        name: "schedule_task",
        description: "Schedule a task. If 'monitor' is true, wait and return logs. If 'useJules' is true, the task will be executed by the Jules sub-agent.",
        inputSchema: {
          type: "object",
          properties: {
            datetime: {
            ...
            extensions: {
              type: "array",
              items: { type: "string" },
              description: "Optional: List of extensions to include.",
            },
            monitor: {
              type: "boolean",
              description: "If true, wait for task completion and return logs.",
              default: false
            },
            useJules: {
              type: "boolean",
              description: "If true, the task will be executed by the Jules sub-agent.",
              default: false
            }
          },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    case "schedule_task": {
      const { datetime, message, name: taskName, extensions, monitor, useJules } = args;
      ...
      if (monitor) {
        ...
        if (result.success) {
          return {
            content: [{ type: "text", text: `Task "${taskName}" completed by ${task.useJules ? 'Jules' : 'Gemini'}.\n\nLogs:\n${result.logs}` }],
          };
        } else {
          return {
      ...
      return {
        content: [{ type: "text", text: `Task "${task.name}" scheduled. Executor: ${task.useJules ? 'Jules' : 'Gemini'}.` }],
      };
    }
    case "list_tasks": {
      return {
    ...
    case "cancel_task": {
      ...
      if (cancelled) {
        ...
        return {
          content: [{ type: "text", text: `Task ${idOrName} not found or already processed.` }],
          isError: true,
        };
      }
    }
    case "monitor_task": {
      ...
      if (result.success) {
        return {
          content: [{ type: "text", text: `SUCCESS: Task "${taskName}" completed.\n\nCaptured Logs:\n${result.logs}` }],
        };
      } else {
        return {
          content: [{ type: "text", text: result.error }],
    ...
    case "check_task_results": {
        ...
        if (fs.existsSync(taskLogPath)) {
          ...
          return {
            content: [{ type: "text", text: `Logs for task "${taskName}":\n\n${content}` }],
          };
        } else {
          return {
            content: [{ type: "text", text: `Log file for task "${taskName}" not found.` }],
            isError: true
          };
        }
    }
